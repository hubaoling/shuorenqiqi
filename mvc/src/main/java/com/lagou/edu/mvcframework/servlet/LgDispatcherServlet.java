        ...
        for(Map.Entry<String,Object> entry: ioc.entrySet()) {
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                if(method.isAnnotationPresent(Security.class)){
                    ...
                    securityUsers = ArrayUtils.addAll(securityUsers,value);
                }

                // 把method所有信息及url封装为一个Handler
                Handler handler = new Handler(entry.getValue(),method, Pattern.compile(url));

                //将url与有权限的用户名存储到securityUsers中
...
//        method.invoke() //
        ...
        String username = getQueryString(req.getQueryString(), "name");
        if(securityUsers != null && securityUsers.length >0){
            if(StringUtils.isEmpty(username)){
                resp.getWriter().write("name is empty");
                return;
            }
            if(!Arrays.asList(securityUsers).contains(username)){
                resp.getWriter().write(username+" can not access");
                return;
            }
        }else{
            resp.getWriter().write(username+" can not access");
